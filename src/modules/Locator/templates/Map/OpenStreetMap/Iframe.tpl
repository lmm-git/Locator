...
<body>
	...
	<p style="text-align:right; font-size: 13px;">
		<input type="radio" name="mapStyle" value="Mapnik" checked="checked" onclick="changeMapDesign('Mapnik');" /> {gt text="Standard"}
		 |
		{*Bug ÖPNV map not working
		<input type="radio" name="mapStyle" value="öpnv" onclick="changeMapDesign('öpnv');" /> {gt text="ÖPNV"}
		 |*}
	...
	<script type="text/javascript">
	  ...
	  var map;
		...
		function changeMapDesign(mapStyle)
		{
			map.destroy();
			map = new OpenLayers.Map ("map", {
			  controls:[
			...
			if (mapStyle == 'öpnv')
			{
				layerTiles = new OpenLayers.Layer.OSM("&Ouml;pnv Welt", "http://tile.memomaps.de/tilegen/${z}/${x}/${y}.png", {numZoomLevels: 19,displayInLayerSwitcher:false,buffer:0});
